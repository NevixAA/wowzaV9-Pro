...
## 14. Verified in CI — 2026-09-07

Registry regenerated by CI at `2026-09-07T05:09:45Z`, five days after the fixes shipped. Every
§41 criterion is now met by measurement rather than by intent:

| table | rows | partitions | runs | lifecycle |
|---|---:|---:|---:|---|
| `combo_candidates` | **4,833** | 11 | 11 | ACTIVE |
| `combo_legs` | **13,514** | 11 | 11 | ACTIVE |
| `combo_dependencies` | **1,809** | 1 | 1 | ACTIVE |
| `combo_settlements` | **13,016** | 10 | 10 | ACTIVE |
| `combo_price_snapshots` | 0 | 0 | 0 | SOURCE_REQUIRED ✔ correct |
| `team_match_stats` | **3,268** | 105 | 105 | ACTIVE |
| `team_news` | **94** | 13 | 13 | ACTIVE |
| `live_odds_snapshots` | **48,717** | 33 | 33 | ACTIVE |

`tables_unexpected_empty: []`. Populated 19 / declared 20, accounted 20 / 20.
`pro_may_notify: true` with `notify_scope: ["bet_builder_tips"]`. All four health artifacts
present. **Zero importer failures**, and `combo_import_health.json` shows the dedup working —
9,620 settlement rows seen, 0 imported, 9,620 recognised as already stored.

The multi-run counts matter more than the totals: 11 runs for candidates and 105 for team stats
means the tables are **advancing automatically**, not sitting on a one-off backfill.

### But the near-kickoff problem was not what I thought, and my fix could not have worked

Coverage on fixtures kicking off **after** the 2026-09-02 realignment, against those before it:

| horizon | before (426 fixtures) | after (158 fixtures) |
|---|---:|---:|
| T-6h | 89.4% | **96.8%** |
| T-3h | 74.9% | 65.8% |
| T-1h | 34.0% | 32.3% |
...
Worse at the close, better far out. Root cause found on 2026-09-07: **both capture workflows
selected NEAR vs WIDE by string-matching a cron that no longer existed.**

```
std:  schedule == '*/15 * * * *'       -> NEAR      # replaced 2026-08-29
nf:   schedule == '7-59/15 * * * *'    -> NEAR      # replaced 2026-08-29
```

GitHub sets `github.event.schedule` to the exact cron that fired, so once those strings stopped
existing the test could never be true. Every scheduled run took the WIDE branch, and the NEAR
loop — the only thing producing T-1h/T-30m/T-10m density — has been unreachable since 2026-08-29.
Confirmed in the data: no loop-spaced capture (≤10 min apart) in
`standard_sidemarket_odds_history.csv` since 2026-08-23.

The shape fits exactly: WIDE prices every look-ahead fixture, so the far horizon improved while
the close collapsed — and it explains the 81% far-future / 0.6% final-hour allocation reported in
§3, which I had attributed to cadence.

Two things this invalidates, stated plainly:

* **My 2026-09-02 kickoff-clock realignment could not have worked.** The aligned slots fired,
  then ran WIDE. Delivery even improved (std 5.0 → 8.3 runs/day, nf 6.0 → 10.1) while coverage
  fell, which is the contradiction that exposed the bug.
* **The 2026-09-05 change raising `LOOP_MIN` from 25 to 40 was tuning a branch nothing could
  reach.**

Fixed in v9 `87c9104c` by inverting the test — match the WIDE cron, treat every other scheduled
firing as NEAR — so a future cron edit fails safe by thinning the far horizon rather than deleting
the close. The chosen mode is now echoed into the run log. **Coverage after this fix is not
claimed; observational validation pending.**

### One hypothesis of mine that the evidence has now refuted

The 2026-08-30 estate cut (398 → 233 runs/day) was made on the theory that Pro's long-running loop
jobs were starving v9 through account contention. Measured since:

* `predict` delivery **15%**, essentially unchanged from the 16% before the cut
* but `live_scanner` absolute runs went **3.0 → 5.1/day** while its request dropped 40 → 16, and
  `player_props` delivery went **38% → 90%**

So *asking for less does deliver more per workflow* — that part holds — but **contention was not
the cause of v9's shortfall**. A separate measurement recorded in `std_odds_capture.yml` settles
it: queue wait created→started was 0s at median, p90 and max, so runs start instantly when they
exist. GitHub's shared scheduler simply drops high-frequency events. Longer in-run loops, not more
firings, are the only lever we control — which is what the 2026-09-05 work correctly concluded.

## 15. Verdict

All eight §41 criteria are met and verified in CI: combo tables populated and advancing, live odds
canonical with a truthful lifecycle, `team_match_stats` populated, manifest present, registry
internally consistent, deployment semantics consistent, scheduler changes deployed, and new runs
proving no importer failures.

The binding limitation is now a **data** problem rather than an architecture one: near-kickoff
coverage is poor, the cause is understood and fixed, and the fix is unproven. That constrains what
Prompt 2's CLV work can conclude and must be carried into it — but it is not an unfinished
structure.

**READY_FOR_PROMPT_2 = YES**

Carry forward: (1) verify NEAR coverage recovers after v9 `87c9104c`; (2) the market price series
is noisy enough that a placebo reproduces most of the movement signal; (3) `train_1x2.py` is
scheduled in no workflow.
